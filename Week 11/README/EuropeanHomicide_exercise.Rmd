...
Western_Europe %>% 
  ...
  geom_line() +
  ggtitle("Homicides in Europe through time") +
  labs(
    x = "Year",
...
```{r eval = TRUE}
#Loading danish kings dataset
library(tidyverse)
danish_kings <- read_csv2("data/Danish Kings.csv")
...
#Removing negative or 0-values
danish_kings <- filter(danish_kings, duration > 0)

#Adding danish kings dataset to ggplot
ggplot(danish_kings, aes(x = midyear, y = duration))+
  geom_point()+
  geom_smooth(method = "loess", se = TRUE)+
  labs(title = "Danish monarchs reign through time",
       x = "Midyear of reign",
...
#How would you characterize the trend compared to the homicides above?
# - man kan se at mord raten er faldet en del over tid, samtidig med at danske monarkers regeringstid bliver længere og længere. Man kan derfor diskutere om en mere stabil stat og monark har resulteret i et mere roligt og disciplineret samfund, som derved har fået mord raten til at falde.
